...
%
% WUC4 - Implements Wu's C4 Compactly Supported Radial Basis Function (RBF)
%
% Description:
%   This class defines Wu's C4 RBF, which is compactly supported and positive definite. 
%   Wu's C4 RBF is commonly used in scattered data interpolation and meshless methods.
%   The compact support ensures efficient computation and sparse system matrices. 
%   The Wu's C4 RBF is defined as:
%       φ(r) = (1 - r/a)^6 * (6 + 36*(r/a) + 82*(r/a)^2 + 72*(r/a)^3 + 30*(r/a)^4 + 5*(r/a)^5), for r ≤ a, and 0 otherwise.
%
% Syntax:
%   obj = WuC4();        % Create an instance of the WuC4 RBF class
%   v = obj.rbf(r, a);   % Evaluate the Wu C4 RBF
%
% Inputs:
...
%
% Example:
%   % Create an instance of WuC4 RBF class
%   phi = WuC4();
%
...
%   xlabel('Distance (r)');
%   ylabel('RBF Value');
%   title('Wu C4 Radial Basis Function');
%
% Methods:
%   rbf(r, a)       - Computes Wu's C4 RBF values for a given distance matrix `r` 
%                     and shape parameter `a`.
%   D1(r, a, x)     - Placeholder for the first derivative of Wu's C4 RBF 
%                     (returns an empty array).
%   D2(r, a, x)     - Placeholder for the second derivative of Wu's C4 RBF 
%                     (returns an empty array).
%   D3(r, a, x)     - Placeholder for the third derivative of Wu's C4 RBF 
%                     (returns an empty array).
%   D4(r, a, x)     - Placeholder for the fourth derivative of Wu's C4 RBF 
%                     (returns an empty array).
...
%
% Notes:
%   - Wu's C4 RBF ensures compact support, leading to sparse system matrices 
...
classdef WuC4 < rbfx
    methods
        function obj = WuC4()  % conatructor
            obj@rbfx();  % call conatructor of the auperclaa 
        end
    
          function v = rbf(obj,r,a)
              v = (1-r./a).^6.*(6 + 36*(r/a) + 82*(r/a).^2 + 72*(r/a).^3 + 30*(r/a).^4 + 5*(r/a).^5);
              v(r>a) = 0.0;
          end % Wu'a C4
